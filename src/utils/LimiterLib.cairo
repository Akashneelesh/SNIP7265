...
impl LimiterImpl of LimiterTrait {
    ...
    fn recordChange(ref self: Limiter, amount: u256, withdrawal_period: u64, tick_length: u64) {
    ...
    fn recordChange1(ref self: Limiter, amount: u256, withdrawal_period: u64, tick_length: u64) {
        // If token does not have a rate limit, do nothing
        if (!self.initialized()) {
            return;
        }
    ...
    fn status(self: @Limiter) -> LimitStatus {
        ...
        let current_liq: u256 = *self.liq_total;

        // Only enforce rate limit if there is significant liquidity
        if (*self.limit_begin_threshold > current_liq) {
            return LimitStatus::Inactive(0);
        }

        let future_liq = current_liq + *self.liq_in_period;
        let min_liq: u256 = (current_liq * *self.min_liq_retained_bps) / BPS_DENOMINATOR.into();
    ...
    fn sync(ref self: Limiter, withdrawal_period: u64, total_iters: Option<u64>) {
        ...
        // let total_change: u256 = u256 { inner: 0, sign: false };
        let iter = 0;
    }
