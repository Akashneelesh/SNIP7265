...
mod CircuitBreaker {
    ...
    #[derive(Drop, starknet::Event)]
    struct NoLockedFunds {}

    #[derive(Drop, starknet::Event)]
    struct RateLimited {}

    #[derive(Drop, starknet::Event)]
    struct GracePeriodStarted {
    ...
    impl CircuitBreakerImpl of ICircuitBreaker<ContractState> {
        ...
        // ) {
        //     if(self._locked_funds.read((_recipient,_asset)) == 0_u256) {
        //         self.emit(Event::NoLockedFunds(NoLockedFunds{}))
        //     }
        //     if(self._is_rate_limited.read() == true) {
        //         self.emit(Event::RateLimited(RateLimited{}))
        //     }

        //     let amount : u256 = self._locked_funds.read((_recipient,_asset));
        ...
        fn addProtectedContracts(
            ...
            loop {
                match data_span.pop_front() {
                    Option::Some(address) => { self._is_protected_contract.write(*address, true); },
                    Option::None(_) => { break; }
                }
            }
        }
    ...
    impl InternalImpl of InternalTrait {
        fn _onTokenInflow(ref self: ContractState, _token: ContractAddress, _amount: u256) {
